          ...
          <div className="space-y-0 mt-16">
            {challenges.map((item, i) => (
              <motion.div key={i} className="border-t border-border" initial={{ opacity: 0, y: 16 }} animate={sec1InView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 + i * 0.06 }}>
                <button className="w-full py-7 flex items-center justify-between gap-6 text-left group transition-all duration-300 hover:pl-6 hover:bg-accent/10 hover:border-l-2 hover:border-l-accent rounded-xl" onClick={() => setOpenChallenge(openChallenge === i ? null : i)} onMouseEnter={() => setOpenChallenge(i)}>
