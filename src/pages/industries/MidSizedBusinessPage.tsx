          ...
          <div className="space-y-0">
            {engagementModels.map((item, i) => (
              <motion.div key={i} className="border-t border-border" initial={{ opacity: 0, y: 16 }} animate={sec5InView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 + i * 0.06 }}>
                <button className="w-full py-7 flex items-center justify-between gap-6 text-left group transition-all duration-300 hover:pl-4 hover:bg-accent/[0.06] rounded-xl" onClick={() => setOpenEngagement(openEngagement === i ? null : i)}>
